...
void sobel_edge_detector(pgm* image, pgm* out_image) {
	...
	#pragma omp parallel for collapse(2) shared(out_image) private(gx,gy) num_threads(MAX_THREADS)
...
void min_max_normalization(pgm* image, int** matrix) {
	...
	#pragma omp parallel for collapse(2) shared(min,max) num_threads(MAX_THREADS)
	for(i = 0; i < image->height; i++) {
		for(j = 0; j < image->width; j++) {
			#pragma omp critical 
			if (matrix[i][j] < min) {
	...
	#pragma omp parallel for collapse(2) shared(max) num_threads(MAX_THREADS)
	for(i = 0; i < image->height; i++) {
		for(j = 0; j < image->width; j++) {
